...
function hal.pack_byte_range(halByteBuffer, length)
	local result = ""
	for i = 1,length do
           --print("rom value ", i, " is ", halByteBuffer[i])
...
function hal.to_hex(number)
	local result = string.format("%x", number)
	if string.len(result) == 1 then
    		result = '0' .. result
      end
      --print('Hex covert of number ', number, ' is ', result)
      return result
end
    

function hal.hex_byte_range(halByteBuffer, length)
	local result = ""
	for i = 1,length do
		result = result .. hal.to_hex(halByteBuffer[i])
     end
     return result
end
