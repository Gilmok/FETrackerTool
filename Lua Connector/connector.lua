...
function processBlock(block)
  ...
  elseif commandType == 0x0F then --read block
    if value < 5000 then
       result['block'] = hal.hex_byte_range(hal.read_byte_range(address, value, domain), value)
    else
...
function receive()
  ...
  while true do
    ...
    if receiveSize ~= 0 then
      --print('received data sz ', receiveSize)
